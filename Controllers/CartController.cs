...
public class CartController : Controller
{
    ...
    public async Task<IActionResult> DatHang(string CustomerName, string Address, string Phone, string? DiscountCode)
    {
        ...
        string[] statuses = { "Đang xử lý", "Đang giao hàng", "Đã giao hàng" };
        Random rand = new Random();
        string randomStatus = statuses[rand.Next(statuses.Length)];
    ...
    public IActionResult CancelOrder(int id)
    {
        ...
        if (order == null)
        {
            TempData["CancelMessage"] = "❌ Không tìm thấy đơn hàng.";
        ...
        if (order.Status == "Đang xử lý")
        {
            ...
            TempData["CancelMessage"] = $"✅ Đơn hàng #{order.Id} đã được hủy thành công.";
        }
        else
        {
            TempData["CancelMessage"] = $"⚠️ Đơn hàng #{order.Id} không thể hủy vì đã được xử lý.";
        }
